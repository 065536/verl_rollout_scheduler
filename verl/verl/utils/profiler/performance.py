...
def gather_all_worker_timings(timing: float) -> list[float]:
    """Gather timing from all workers and return the complete list.
    
    This function uses all_gather to collect timing from all workers,
    allowing access to individual worker timings instead of just min/max.
    
    Args:
        timing: The timing value for the current worker/rank
        
    Returns:
        List of timing values for all workers (ordered by rank)
    """
    if not dist.is_initialized():
        return [timing]
    ...
    # Convert to list of floats, ordered by rank
    all_timings = [tensor.item() for tensor in tensor_list]
    return all_timings
