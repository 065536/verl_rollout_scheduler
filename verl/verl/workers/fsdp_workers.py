...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        # DIRECT MEASUREMENT: Record start time before generation (for verification)
        # This ensures we have a reliable wall-clock time measurement
        import time
        generation_start_time = time.perf_counter()  # Use perf_counter for higher precision
        
        with simple_timer("generate_sequences", timing_generate):
            output = self.rollout.generate_sequences(prompts=prompts)
        
        # DIRECT MEASUREMENT: Record end time after generation
        generation_end_time = time.perf_counter()
        direct_measured_time = generation_end_time - generation_start_time
        
        # Verify that simple_timer time matches direct measurement
        # This helps ensure timing measurements are accurate
        timer_time = timing_generate.get("generate_sequences", 0.0)
        if timer_time > 0:
            time_diff = abs(timer_time - direct_measured_time)
            time_diff_ratio = time_diff / timer_time if timer_time > 0 else 0
            
            # Warn if difference is significant (>5% or >0.5s)
            if time_diff_ratio > 0.05 or time_diff > 0.5:
                logger.warning(
                    f"[TIMING VERIFICATION] Worker {self.rank}: "
                    f"timer_time={timer_time:.4f}s, direct_time={direct_measured_time:.4f}s, "
                    f"diff={time_diff:.4f}s ({time_diff_ratio*100:.2f}%)"
                )
        ...
        # Gather all worker timings (not just min/max) - NEW: allows per-worker analysis
        # IMPORTANT: This collects the ACTUAL time each worker spent in generate_sequences
        # Each worker independently measured its own time using simple_timer context manager
        from verl.utils.profiler.performance import gather_all_worker_timings
        all_worker_timings = gather_all_worker_timings(timing_generate["generate_sequences"])
        ...
        timing_generate.update(
            {
                ...
                "generation_timing/all_workers": all_worker_timings,  # NEW: List of all worker timings
            }
        )
